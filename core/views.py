...
class CollegeChatbotView(View):
    def post(self, request):
        try:
            ...
            context = "Information about Sree Sankara Vidyapeetom College:\n"
            for qna in college_info.get('Sheet1', []):
                if qna.get('QUESTIONS') and qna.get('ANSWER'):
                    context += f"Q: {qna['QUESTIONS']}\nA: {qna['ANSWER']}\n\n"
            
            system_prompt = f"""
            You are a helpful and polite virtual assistant for Sree Sankara Vidyapeetom College. 
            Use ONLY the following context to answer the user's questions. 
            If the answer is not in the context, politely state that you only have information about the college's basic details and suggest them to contact the college office at 04702678899 or email ssvpcn@gmail.com.
